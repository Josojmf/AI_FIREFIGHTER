...
async def get_dashboard_stats(user_data: Dict = Depends(require_user)):
    """Estadísticas generales del dashboard (cualquier usuario autenticado)"""
    try:
        # Usuarios
...
async def get_users_stats(user_data: Dict = Depends(require_user)):
    """Estadísticas detalladas de usuarios (cualquier usuario autenticado)"""
    try:
...
async def get_cards_stats(user_data: Dict = Depends(require_user)):
    """Estadísticas detalladas de memory cards (cualquier usuario autenticado)"""
    try:
        ...
        return {
            ...
            "stats": {
                ...
                "avg_reviews_per_card": round(total_reviews / total, 2)
                if total > 0
                else 0,
            },
        }
...
async def get_tokens_stats(user_data: Dict = Depends(require_user)):
    """Estadísticas detalladas de access tokens (cualquier usuario autenticado)"""
    try:
        ...
        return {
            ...
            "stats": {
                ...
                "usage_rate": round(
                    (total_uses / total_max_uses * 100), 2
                )
                if total_max_uses > 0
                else 0,
            },
...
async def clear_system_cache(user_data: Dict = Depends(require_user)):
    """Limpiar cache del sistema (cualquier usuario autenticado)"""
    try:
        clear_cache()
        return {
            "ok": True,
...
async def create_system_backup(user_data: Dict = Depends(require_user)):
    """Crear backup del sistema (cualquier usuario autenticado)"""
    try:
